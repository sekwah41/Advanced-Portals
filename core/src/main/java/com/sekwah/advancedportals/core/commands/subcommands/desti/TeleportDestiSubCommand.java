...
public class TeleportDestiSubCommand implements SubCommand {
    ...
    public void onCommand(CommandSenderContainer sender, String[] args) {
        if (args.length > 1) {
            if(args.length > 2 && Permissions.TELEPORT_PLAYER_DESTI.hasPermission(sender)) {
                ...
                if (destinationServices.teleportToDestination(args[1], targetPlayer)) {
                    sender.sendMessage(
                            Lang.getPositivePrefix()
                                    + Lang.translateInsertVariables("command.destination.teleportplayer.success", args[2], args[1]));

                    targetPlayer.sendMessage(Lang.getPositivePrefix()
                            + Lang.translateInsertVariables("command.destination.teleport.success", args[1])
                            // leave this in case they have the old translation strings
                            .replaceAll("@destiname", args[1]));
                } else {
                    sender.sendMessage(
                            Lang.getNegativePrefix()
                                    + Lang.translateInsertVariables("command.destination.teleportplayer.error", args[2], args[1]));
                }
            } else {
                if (destinationServices.teleportToDestination(
                    ...
                    sender.sendMessage(
                            Lang.getNegativePrefix()
                                    + Lang.translateInsertVariables("command.destination.teleport.error", args[1])
                                    // leave this in case they have the old translation strings
                                    .replaceAll("@destiname", args[1]));
                }
            }
            ...
            sender.sendMessage(Lang.getNegativePrefix()
                    // should probably be translateInsertVariables but this predates that. Leave it for now
    ...
    public List<String> onTabComplete(CommandSenderContainer sender,
        ...
        if (args.length == 2) {
            ...
            return destiNames;
        } else if(args.length == 3 && Permissions.TELEPORT_PLAYER_DESTI.hasPermission(sender)) {
